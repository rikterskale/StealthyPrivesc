...
impl Plugin for SystemdCronPlugin {
    ...
    fn run(&self, ctx: &mut PluginContext<'_>) -> Result<Vec<Finding>> {
        ...
        for dir in unit_dirs {
            check_writable_tree(dir, euid, "systemd unit path", &mut findings);
            scan_timers(dir, euid, &mut findings);
        }
        ...
        for p in cron_paths {
            check_writable_tree(p, euid, "cron path", &mut findings);
        }
...
fn scan_timers(dir: &str, euid: u32, findings: &mut Vec<Finding>) {
    let p = Path::new(dir);
    if !p.is_dir() {
        return;
    }
...
fn check_writable_tree(path: &str, euid: u32, label: &str, findings: &mut Vec<Finding>) {
    let p = Path::new(path);
...
fn is_writable_by(meta: &fs::Metadata, euid: u32) -> bool {
    let mode = meta.permissions().mode();
    if euid == 0 {
        return false; // already root; not interesting as an escalation path
    }

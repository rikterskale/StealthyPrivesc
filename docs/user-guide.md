...
| Binary blocked | Record via `*.endpoint_controls` / script fallbacks; use `enum.sh`/`enum.py` or `enum.ps1`/`enum.js`. `--allow-techniques endpoint-bypass` is scaffold-only. |

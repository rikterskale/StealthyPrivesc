...
$argsToRun = if ($Arguments) { $Arguments } else { @('--authorized','--profile','balanced','enum') }
if (($argsToRun -notcontains '--authorized') -and ($argsToRun -notcontains '--i-understand-authorized-use-only')) {
  $argsToRun = @('--authorized') + $argsToRun
}
